...
// Branded post-image renderer (next/og / Satori → PNG). Shared by the /template
// route (for GET/POST previews) AND lib/image.ts (called DIRECTLY, no self-fetch —
// a server self-fetch to our own deployment URL hit Vercel's auth wall and returned
// an HTML page that got saved as a ".png", which is what broke image generation).

export interface TemplateOpts {
  bg?: string;        // AI background: public URL or data:image/...;base64 (optional)
  layout?: string;    // announcement | stat | quote | filing
  ticker?: string;    // drives the brand kit (e.g. AMFN)
...
export function renderPostTemplate(o: TemplateOpts): ImageResponse {
  const bg = o.bg || "";
  ...
  // Container always gets the brand gradient (Satori renders linear-gradients). Raster
  // AI art is layered as a full-bleed <img> below (Satori paints images via <img>, not
  // CSS backgroundImage).
