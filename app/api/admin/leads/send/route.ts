...
export async function POST(req: Request) {
  ...
  const { count: sentToday } = await svc.from("leads").select("id", { count: "exact", head: true }).gte("last_sent_at", since);
  ...
  let q = svc.from("leads").select("*").eq("list_id", listId).neq("email", "").in("status", ["new", "queued"]);
  ...
  for (const lead of batch) {
    try {
      ...
      await svc.from("leads").update({ status: "sent", message_id: messageId ?? "", last_sent_at: new Date().toISOString() }).eq("id", lead.id);
      sent++;
    } catch (e) {
