...
export async function POST(req: Request) {
  ...
  switch (event.type) {
    ...
    case "customer.subscription.deleted": {
      ...
      await svc.from("members").update(patch).eq("stripe_subscription_id", sub.id);
      break;
    }
    case "invoice.payment_failed": {
      const inv = event.data.object as { subscription?: string };
      if (inv.subscription) await svc.from("members").update({ subscription_status: "past_due" }).eq("stripe_subscription_id", inv.subscription);
      break;
    }
  }
