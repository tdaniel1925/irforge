...
export default function WorkspacePage() {
  ...
  const load = async () => {
    try { const d = await (await fetch("/api/workspace")).json(); setNotes(d.notes ?? []); }
  ...
  const save = async () => {
    ...
    setBusy(true);
    try {
      const d = await (await fetch("/api/workspace", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(editing) })).json();
      if (d.note) {
  ...
  const del = async (id: string) => {
    ...
    await fetch(`/api/workspace?id=${encodeURIComponent(id)}`, { method: "DELETE" });
    setNotes((ns) => ns.filter((n) => n.id !== id));
  };

  const togglePin = async (n: Note) => {
    const d = await (await fetch("/api/workspace", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...n, pinned: !n.pinned }) })).json();
    if (d.note) await load();
  };
