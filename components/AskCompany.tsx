...
export default function AskCompany({ ticker, claimed }: { ticker: string; claimed: boolean }) {
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-40 border-t border-slate-700 bg-slate-900/95 backdrop-blur supports-[backdrop-filter]:bg-slate-900/80">
            ...
            <div className="flex items-center gap-2">
              {expanded && (
                <input
                  ...
                  className="hidden w-40 rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-slate-200 focus:border-emerald-500 focus:outline-none sm:block"
                />
              )}
              <input
                value={question}
